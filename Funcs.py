...
def xi_square(sample: list) -> tuple:
    """
    Вычисление критерия Хи-квадрата.
    """
    a = 0
    theta = 10000
    ...
    for i in range(len(intervals) - 1):
        ...
        if intervals[i+1] == right and right != 10000:
            right -= 1
...
def gen_params(sample: list):
    """
    Проверка на равномерность распределения и случайность, используя критерий Хи-квадрат.
    """

    norm_sample = [i / 10000 for i in sample] # нормированная выборка
    mean = average(norm_sample) # выборочное среднее
    dispersion = variance(norm_sample) # выборочное среднее
